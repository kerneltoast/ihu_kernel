...
static int nxp_probe(struct phy_device *phydev)
{
	...
	mutex_init(&nxp_specific->cnt_mutex);
	...
	if (err) {
		dev_err(&phydev->mdio.dev, "nxp_attribute_group creation failed\n");
		goto register_sysfs_error;
	}

	err = sysfs_create_group(&phydev->mdio.dev.kobj, &nxp_stats_attribute_group);
	if (err) {
		dev_err(&phydev->mdio.dev, "nxp_stats_attribute_group creation failed\n");
		goto register_stats_sysfs_error;
	}

	return 0;

/* error handling */
register_stats_sysfs_error:
...
static void nxp_remove(struct phy_device *phydev)
{
	...
	sysfs_remove_group(&phydev->mdio.dev.kobj, &nxp_attribute_group);
	sysfs_remove_group(&phydev->mdio.dev.kobj, &nxp_stats_attribute_group);
 ...
 */
static bool update_counters(void * priv, struct phy_device *phydev, int interrupts)
{
	struct nxp_specific_data * nxp_specific = (struct nxp_specific_data *) priv;
	volatile int link_fail_cnt;
	volatile int comm_stat;
	volatile int ext_status;
	int loc_rcvr_fail;
	int rem_rcvr_fail;

	if ((nxp_specific == NULL) || (phydev == NULL))
		goto fail_exit;

	/*
	 * read out the value from registers MII_LINK_FAIL_COUNTER, MII_COMMSTAT and
	 * MII_EXTERNAL_STATUS
	 */

	link_fail_cnt = phy_read(phydev, MII_LINK_FAIL_COUNTER);
	if (link_fail_cnt < 0) {
		dev_err(&phydev->mdio.dev, "phy_read MII_LINK_FAIL_COUNTER failed\n");
		link_fail_cnt = 0;
	}

	comm_stat = phy_read(phydev, MII_COMMSTAT);
	if (comm_stat < 0){
		dev_err(&phydev->mdio.dev, "phy_read MII_COMMSTAT failed\n");
		comm_stat = 0;
	}

	ext_status = phy_read(phydev, MII_EXTERNAL_STATUS);
	if (ext_status < 0){
		dev_err(&phydev->mdio.dev, "phy_read MII_EXTERNAL_STATUS failed\n");
		ext_status = 0;
	}

	mutex_lock(&nxp_specific->cnt_mutex);

	if (interrupts & INTERRUPT_LINK_STATUS_UP) {
		/* update link up counter */
		nxp_specific->phy_counters.link_status_up ++;
	}

	if (interrupts & INTERRUPT_LINK_STATUS_FAIL) {
		/* update link fail counter */
		nxp_specific->phy_counters.link_status_fail ++;

		/*
		 * update Statistics For PHY Receiver Fails
		 *
		 * PHY Receiver Fails counter are stored in register:
		 * [26] LOC_RCVR_CNT (bits 15:8) : Local Receiver Fails counter
		 * [26] REM_RCVR_CNT (bits 7:0)  : Remote Receiver fails counter
		 *
		 * Counters in register [R26] are incremented to value 255. Register
		 * [R26] is cleared after read.
		 */
		loc_rcvr_fail = (link_fail_cnt & LINK_FAIL_LOC_RCVR_CNT_MSK)
		                >> LINK_FAIL_LOC_RCVR_CNT_SHIFT;
		nxp_specific->phy_counters.loc_rcvr_fail += loc_rcvr_fail;

		rem_rcvr_fail = (link_fail_cnt & LINK_FAIL_REM_RCVR_CNT_MSK)
		                >> LINK_FAIL_REM_RCVR_CNT_SHIFT;
		nxp_specific->phy_counters.rem_rcvr_fail += rem_rcvr_fail;
	}

	if (interrupts & INTERRUPT_SNR_WARNING) {
		/*
		 * update Statistics For SNR Warning
		 */
		nxp_specific->phy_counters.sqi_warn ++;
	}

	if (interrupts & INTERRUPT_CONTROL_ERROR) {
		/*
		 * update Statistics For SMI control error
		 */
		nxp_specific->phy_counters.mii_err ++;
	}

	if (interrupts & INTERRUPT_SYM_ERR) {
		/*
		 * update Statistics For Symbol error
		 */
		nxp_specific->phy_counters.symbol_err ++;
	}

	if (interrupts & INTERRUPT_PHY_INIT_FAIL) {
		/*
		 * update Statistics For Phy Init error
		 */
		nxp_specific->phy_counters.phy_init_fail ++;
	}

	if ((interrupts & INTERRUPT_LINK_STATUS_FAIL)
	    || (interrupts & INTERRUPT_SYM_ERR))

	{
		/**
		 * update Statistics For PHY Misc Errors
		 *
		 * PHY Misc Errors are stored in register:
		 * [23] SSD_ERR   : SSD error detected
		 * [23] ESD_ERR   : ESD error detected
		 *
		 * Flags are cleared after registers read or when link control is
		 * disabled.
		 */

		if (comm_stat & COMMSTAT_SSD_ERROR)
			nxp_specific->phy_counters.ssd_err ++;
		if (comm_stat & COMMSTAT_ESD_ERROR)
			nxp_specific->phy_counters.esd_err ++;
	}

	if (interrupts & INTERRUPT_TEMP_ERROR) {
		/*
		 * update Statistics For Temperature error
		 */
		nxp_specific->phy_counters.temp_err ++;

		/*
		 * PHY Temperature Errors are stored in register:
		 * [25] TEMP_HIGH : temperature above high level
		 * [25] TEMP_WARN : temperature above warning level
		 */
		if (ext_status & EXTSTAT_TEMP_WARN)
			nxp_specific->phy_counters.temp_warn ++;
		if (ext_status & EXTSTAT_TEMP_HIGH)
			nxp_specific->phy_counters.temp_high ++;
	}

	if (interrupts & INTERRUPT_UV_ERR) {
		/*
		 * update Statistics For Undervoltage error
		 */
		nxp_specific->phy_counters.uv_err ++;

		/*
		* PHY Undervoltage Errors are stored in register:
		* [25] UV_VDDA_3V3  : undervoltage detected on pin VDDA(3V3)
		* [25] UV_VDDD_1V8  : undervoltage detected on pin VDDD(1V8)
		* [25] UV_VDDA_1V8  : undervoltage detected on pin VDDA(1V8)
		* [25] UV_VDDIO     : undervoltage detected on pin VDD(IO)
		 */
		if (ext_status & EXTSTAT_UV_VDDIO)
			nxp_specific->phy_counters.uv_vddio ++;
		if (ext_status & EXTSTAT_UV_VDDA_1V8)
			nxp_specific->phy_counters.uv_vdda_1v8 ++;
		if (ext_status & EXTSTAT_UV_VDDD_1V8)
			nxp_specific->phy_counters.uv_vddd_1v8 ++;
		if (ext_status & EXTSTAT_UV_VDDA_3V3)
			nxp_specific->phy_counters.uv_vdda_3v3 ++;
	}

	if (interrupts & INTERRUPT_ERROR_EVENT) {
		/*
		 * update Statistics For PHY Transmission Errors
		 *
		 * PHY Transmission Errors are stored in register:
		 * [23] RECEIVE_ERR  (bit 4) : Receive Error detected since last read
		 * [23] TRANSMIT_ERR (bit 3) : Transmit Error detected since last read
		 *
		 * Flags are cleared after registers read or when link control is
		 * disabled.
		 */

		if (comm_stat & COMMSTAT_RECEIVE_ERROR)
			nxp_specific->phy_counters.trans_rx_err ++;
		if (comm_stat & COMMSTAT_TRANSMIT_ERROR)
			nxp_specific->phy_counters.trans_tx_err ++;
	}

	mutex_unlock(&nxp_specific->cnt_mutex);
...
static void poll(struct work_struct *work)
{
	...
	if ((interrupts != 0xffff) && (interrupt_mask >= 0)) {
		...
		/* update statistics counters */
		if ((interrupts & INTERRUPT_LINK_STATUS_UP) ||
		    (interrupts & INTERRUPT_ERROR_EVENT) ||
		    (interrupts & INTERRUPT_SNR_WARNING)) {
				if (update_counters(phydev->priv, phydev, interrupts) != 0)
					dev_err(&phydev->mdio.dev, "PHY counters update failed\n");
			}
...
static int nxp_suspend(struct phy_device *phydev)
{
	...
	if( ((struct nxp_specific_data *)phydev->priv)->poll_setup == 1) {
...
static int nxp_resume(struct phy_device *phydev)
{
	...
	if( ((struct nxp_specific_data *)phydev->priv)->poll_setup == 0) {
		/* Configure PHY and restore interrrupts */
	        phydev->drv->config_init(phydev);
	}
...
{
	/* Make sure poll_setup is running before aneg */
	if( ((struct nxp_specific_data *)phydev->priv)->poll_setup == 0) {
	        nxp_resume(phydev);
	}
...
/* This function handles read accesses to the 'sqi_wlimit' node in sysfs.
 * Reading the node returns the current sqi_wlimit setting.
 */
static ssize_t sysfs_get_sqi_wlimit(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	struct phy_device *phydev = container_of(dev, struct phy_device, mdio.dev);
	volatile int value;

	value = phy_read(phydev, MII_CFG2);
	if (value < 0)
		dev_err(&phydev->mdio.dev, "failed to read register MII_CFG2\n");

	value = (value & CFG2_SNR_WLIMIT) >> CFG2_SNR_WLIMIT_SHIFT;

	return sprintf(buf, "%d\n", value);
}

/* This function handles write accesses to the 'sqi_wlimit' node in sysfs.
 * Writing to the node updates the sqi_wlimit setting.
 */
static ssize_t sysfs_set_sqi_wlimit(struct device *dev,
				   struct device_attribute *attr,
				   const char *buf, size_t count)
{
	int err;
	...
	if (err < 0) {
		dev_err(&phydev->mdio.dev, "sysfs_set_sqi_wlimit: parse error\n");
		return err;
	}

	if ((value > CFG2_SNR_WLIMIT_MAX) || (value < 0)) {
		dev_err(&phydev->mdio.dev, "sysfs_set_sqi_wlimit: invalid setting %d\n", value);
		return -EINVAL;
	}

	reg_val = phy_read(phydev, MII_CFG2);
	if (reg_val < 0) {
		dev_err(&phydev->mdio.dev, "sysfs_set_sqi_wlimit: phy_read failed\n");
		return reg_val;
	}

	reg_val &= ~CFG2_SNR_WLIMIT;
	reg_val |= value << CFG2_SNR_WLIMIT_SHIFT;

	err = phy_write(phydev, MII_CFG2, reg_val);
	if (err < 0) {
		dev_err(&phydev->mdio.dev, "sysfs_set_sqi_wlimit: phy_write failed\n");
		return err;
	}
...
/* This function handles read accesses to the node in stats sysfs.
 * Reading the node returns the current counter.
 */
static ssize_t sysfs_get_statistics_counter(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	struct phy_device *phydev = container_of(dev, struct phy_device, mdio.dev);
	struct nxp_specific_data * nxp_specific = (struct nxp_specific_data *) phydev->priv;

	/* write result into the buffer */
	if (!strncmp(attr->attr.name, "link_status_up", strlen(attr->attr.name))) {
		return sprintf(buf, "%d\n", nxp_specific->phy_counters.link_status_up);
	}
	else if (!strncmp(attr->attr.name, "link_status_fail", strlen(attr->attr.name))) {
		return sprintf(buf, "%d\n", nxp_specific->phy_counters.link_status_fail);
	}
	else if (!strncmp(attr->attr.name, "loc_rcvr_fail", strlen(attr->attr.name))) {
		return sprintf(buf, "%d\n", nxp_specific->phy_counters.loc_rcvr_fail);
	}
	else if (!strncmp(attr->attr.name, "rem_rcvr_fail", strlen(attr->attr.name))) {
		return sprintf(buf, "%d\n", nxp_specific->phy_counters.rem_rcvr_fail);
	}
	else if (!strncmp(attr->attr.name, "trans_rx_err", strlen(attr->attr.name))) {
		return sprintf(buf, "%d\n", nxp_specific->phy_counters.trans_rx_err);
	}
	else if (!strncmp(attr->attr.name, "trans_tx_err", strlen(attr->attr.name))) {
		return sprintf(buf, "%d\n", nxp_specific->phy_counters.trans_tx_err);
	}
	else if (!strncmp(attr->attr.name, "sqi_warn", strlen(attr->attr.name))) {
		return sprintf(buf, "%d\n", nxp_specific->phy_counters.sqi_warn);
	}
	else if (!strncmp(attr->attr.name, "mii_err", strlen(attr->attr.name))) {
		return sprintf(buf, "%d\n", nxp_specific->phy_counters.mii_err);
	}
	else if (!strncmp(attr->attr.name, "phy_init_fail", strlen(attr->attr.name))) {
		return sprintf(buf, "%d\n", nxp_specific->phy_counters.phy_init_fail);
	}
	else if (!strncmp(attr->attr.name, "symbol_err", strlen(attr->attr.name))) {
		return sprintf(buf, "%d\n", nxp_specific->phy_counters.symbol_err);
	}
	else if (!strncmp(attr->attr.name, "ssd_err", strlen(attr->attr.name))) {
		return sprintf(buf, "%d\n", nxp_specific->phy_counters.ssd_err);
	}
	else if (!strncmp(attr->attr.name, "esd_err", strlen(attr->attr.name))) {
		return sprintf(buf, "%d\n", nxp_specific->phy_counters.esd_err);
	}
	else if (!strncmp(attr->attr.name, "temp_err", strlen(attr->attr.name))) {
		return sprintf(buf, "%d\n", nxp_specific->phy_counters.temp_err);
	}
	else if (!strncmp(attr->attr.name, "temp_high", strlen(attr->attr.name))) {
		return sprintf(buf, "%d\n", nxp_specific->phy_counters.temp_high);
	}
	else if (!strncmp(attr->attr.name, "temp_warn", strlen(attr->attr.name))) {
		return sprintf(buf, "%d\n", nxp_specific->phy_counters.temp_warn);
	}
	else if (!strncmp(attr->attr.name, "uv_err", strlen(attr->attr.name))) {
		return sprintf(buf, "%d\n", nxp_specific->phy_counters.uv_err);
	}
	else if (!strncmp(attr->attr.name, "uv_vdda_3v3", strlen(attr->attr.name))) {
		return sprintf(buf, "%d\n", nxp_specific->phy_counters.uv_vdda_3v3);
	}
	else if (!strncmp(attr->attr.name, "uv_vddd_1v8", strlen(attr->attr.name))) {
		return sprintf(buf, "%d\n", nxp_specific->phy_counters.uv_vddd_1v8);
	}
	else if (!strncmp(attr->attr.name, "uv_vdda_1v8", strlen(attr->attr.name))) {
		return sprintf(buf, "%d\n", nxp_specific->phy_counters.uv_vdda_1v8);
	}
	else if (!strncmp(attr->attr.name, "uv_vddio", strlen(attr->attr.name))) {
		return sprintf(buf, "%d\n", nxp_specific->phy_counters.uv_vddio);
	}
	else {
		dev_err(&phydev->mdio.dev, "Unknow attr name %s\n", attr->attr.name);
	}
	return 0;
}

/* This function handles write accesses to the node in stats sysfs.
 * Writing to the node updates the statistics counters.
 */
static ssize_t sysfs_set_statistics_counter(struct device *dev,
				   struct device_attribute *attr,
				   const char *buf, size_t count)
{
	int err;
	...
	err = kstrtoint(buf, 10, &value);
	if (err < 0) {
		dev_err(&phydev->mdio.dev, "parse error: sysfs_set_statistics_counter failed\n");
		return err;
	}

	/* write value to the statistics counters */
	mutex_lock(&nxp_specific->cnt_mutex);

	if (!strncmp(attr->attr.name, "link_status_up", strlen(attr->attr.name))) {
		nxp_specific->phy_counters.link_status_up = value;
	}
	else if (!strncmp(attr->attr.name, "link_status_fail", strlen(attr->attr.name))) {
		nxp_specific->phy_counters.link_status_fail = value;
	}
	else if (!strncmp(attr->attr.name, "loc_rcvr_fail", strlen(attr->attr.name))) {
		nxp_specific->phy_counters.loc_rcvr_fail = value;
	}
	else if (!strncmp(attr->attr.name, "rem_rcvr_fail", strlen(attr->attr.name))) {
		nxp_specific->phy_counters.rem_rcvr_fail = value;
	}
	else if (!strncmp(attr->attr.name, "trans_rx_err", strlen(attr->attr.name))) {
		nxp_specific->phy_counters.trans_rx_err = value;
	}
	else if (!strncmp(attr->attr.name, "trans_tx_err", strlen(attr->attr.name))) {
		nxp_specific->phy_counters.trans_tx_err = value;
	}
	else if (!strncmp(attr->attr.name, "sqi_warn", strlen(attr->attr.name))) {
		nxp_specific->phy_counters.sqi_warn = value;
	}
	else if (!strncmp(attr->attr.name, "mii_err", strlen(attr->attr.name))) {
		nxp_specific->phy_counters.mii_err = value;
	}
	else if (!strncmp(attr->attr.name, "phy_init_fail", strlen(attr->attr.name))) {
		nxp_specific->phy_counters.phy_init_fail = value;
	}
	else if (!strncmp(attr->attr.name, "symbol_err", strlen(attr->attr.name))) {
		nxp_specific->phy_counters.symbol_err = value;
	}
	else if (!strncmp(attr->attr.name, "ssd_err", strlen(attr->attr.name))) {
		nxp_specific->phy_counters.ssd_err = value;
	}
	else if (!strncmp(attr->attr.name, "esd_err", strlen(attr->attr.name))) {
		nxp_specific->phy_counters.esd_err = value;
	}
	else if (!strncmp(attr->attr.name, "temp_err", strlen(attr->attr.name))) {
		nxp_specific->phy_counters.temp_err = value;
	}
	else if (!strncmp(attr->attr.name, "temp_high", strlen(attr->attr.name))) {
		nxp_specific->phy_counters.temp_high = value;
	}
	else if (!strncmp(attr->attr.name, "temp_warn", strlen(attr->attr.name))) {
		nxp_specific->phy_counters.temp_warn = value;
	}
	else if (!strncmp(attr->attr.name, "uv_err", strlen(attr->attr.name))) {
		nxp_specific->phy_counters.uv_err = value;
	}
	else if (!strncmp(attr->attr.name, "uv_vdda_3v3", strlen(attr->attr.name))) {
		nxp_specific->phy_counters.uv_vdda_3v3 = value;
	}
	else if (!strncmp(attr->attr.name, "uv_vddd_1v8", strlen(attr->attr.name))) {
		nxp_specific->phy_counters.uv_vddd_1v8 = value;
	}
	else if (!strncmp(attr->attr.name, "uv_vdda_1v8", strlen(attr->attr.name))) {
		nxp_specific->phy_counters.uv_vdda_1v8 = value;
	}
	else if (!strncmp(attr->attr.name, "uv_vddio", strlen(attr->attr.name))) {
		nxp_specific->phy_counters.uv_vddio = value;
	}
	else {
		mutex_unlock(&nxp_specific->cnt_mutex);
		dev_err(&phydev->mdio.dev, "Unknow attr name %s\n", attr->attr.name);
...
static DEVICE_ATTR(sqi_wlimit, S_IWUSR | S_IRUSR,
		   sysfs_get_sqi_wlimit, sysfs_set_sqi_wlimit);
static DEVICE_ATTR(link_status_up, S_IWUSR | S_IRUSR,
		   sysfs_get_statistics_counter, sysfs_set_statistics_counter);
static DEVICE_ATTR(link_status_fail, S_IWUSR | S_IRUSR,
		   sysfs_get_statistics_counter, sysfs_set_statistics_counter);
static DEVICE_ATTR(loc_rcvr_fail, S_IWUSR | S_IRUSR,
		   sysfs_get_statistics_counter, sysfs_set_statistics_counter);
static DEVICE_ATTR(rem_rcvr_fail, S_IWUSR | S_IRUSR,
		   sysfs_get_statistics_counter, sysfs_set_statistics_counter);
static DEVICE_ATTR(trans_rx_err, S_IWUSR | S_IRUSR,
		   sysfs_get_statistics_counter, sysfs_set_statistics_counter);
static DEVICE_ATTR(trans_tx_err, S_IWUSR | S_IRUSR,
		   sysfs_get_statistics_counter, sysfs_set_statistics_counter);
static DEVICE_ATTR(sqi_warn, S_IWUSR | S_IRUSR,
		   sysfs_get_statistics_counter, sysfs_set_statistics_counter);
static DEVICE_ATTR(mii_err, S_IWUSR | S_IRUSR,
		   sysfs_get_statistics_counter, sysfs_set_statistics_counter);
static DEVICE_ATTR(phy_init_fail, S_IWUSR | S_IRUSR,
		   sysfs_get_statistics_counter, sysfs_set_statistics_counter);
static DEVICE_ATTR(symbol_err, S_IWUSR | S_IRUSR,
		   sysfs_get_statistics_counter, sysfs_set_statistics_counter);
static DEVICE_ATTR(ssd_err, S_IWUSR | S_IRUSR,
		   sysfs_get_statistics_counter, sysfs_set_statistics_counter);
static DEVICE_ATTR(esd_err, S_IWUSR | S_IRUSR,
		   sysfs_get_statistics_counter, sysfs_set_statistics_counter);
static DEVICE_ATTR(temp_err, S_IWUSR | S_IRUSR,
		   sysfs_get_statistics_counter, sysfs_set_statistics_counter);
static DEVICE_ATTR(temp_high, S_IWUSR | S_IRUSR,
		   sysfs_get_statistics_counter, sysfs_set_statistics_counter);
static DEVICE_ATTR(temp_warn, S_IWUSR | S_IRUSR,
		   sysfs_get_statistics_counter, sysfs_set_statistics_counter);
static DEVICE_ATTR(uv_err, S_IWUSR | S_IRUSR,
		   sysfs_get_statistics_counter, sysfs_set_statistics_counter);
static DEVICE_ATTR(uv_vdda_3v3, S_IWUSR | S_IRUSR,
		   sysfs_get_statistics_counter, sysfs_set_statistics_counter);
static DEVICE_ATTR(uv_vddd_1v8, S_IWUSR | S_IRUSR,
		   sysfs_get_statistics_counter, sysfs_set_statistics_counter);
static DEVICE_ATTR(uv_vdda_1v8, S_IWUSR | S_IRUSR,
		   sysfs_get_statistics_counter, sysfs_set_statistics_counter);
static DEVICE_ATTR(uv_vddio, S_IWUSR | S_IRUSR,
		   sysfs_get_statistics_counter, sysfs_set_statistics_counter);
...
static struct attribute *nxp_sysfs_entries[] = {
	...
	&dev_attr_sqi_wlimit.attr,
	NULL
};

static struct attribute *nxp_stats_sysfs_entries[] = {
	&dev_attr_link_status_up.attr,
	&dev_attr_link_status_fail.attr,
	&dev_attr_loc_rcvr_fail.attr,
	&dev_attr_rem_rcvr_fail.attr,
	&dev_attr_trans_rx_err.attr,
	&dev_attr_trans_tx_err.attr,
	&dev_attr_sqi_warn.attr,
	&dev_attr_mii_err.attr,
	&dev_attr_phy_init_fail.attr,
	&dev_attr_symbol_err.attr,
	&dev_attr_ssd_err.attr,
	&dev_attr_esd_err.attr,
	&dev_attr_temp_err.attr,
	&dev_attr_temp_high.attr,
	&dev_attr_temp_warn.attr,
	&dev_attr_uv_err.attr,
	&dev_attr_uv_vdda_3v3.attr,
	&dev_attr_uv_vddd_1v8.attr,
	&dev_attr_uv_vdda_1v8.attr,
	&dev_attr_uv_vddio.attr,
	NULL
};
...
static struct attribute_group nxp_stats_attribute_group = {
	.name = "stats",
	.attrs = nxp_stats_sysfs_entries,
};
...
static struct phy_device *search_phy_by_addr(int phy_addr)
{
	...
	while (dev) {
		if (dev->phydev) {
			...
			phydev = mdiobus_get_phy(dev->phydev->mdio.bus, phy_addr);
			if (phydev)
				return phydev;
		}
...
static struct phy_device *search_mdio_by_id(struct mii_bus *bus, int phy_id)
{
	...
	for (addr = 0; addr < PHY_MAX_ADDR; addr++) {
		if (mdiobus_get_phy(bus, addr)) {
			phydev = mdiobus_get_phy(bus, addr);
			if ((phydev->phy_id & NXP_PHY_ID_MASK) == phy_id) {
				pr_info("NXP PHY: found the given phy\n");
				return phydev;
			}
		}
	}
...
static struct phy_device *search_phy_by_id(int phy_id)
{
	...
	while (dev) {
		if (dev->phydev)
			return search_mdio_by_id(dev->phydev->mdio.bus, phy_id);
		dev = next_net_device(dev);
	}
	return NULL;
}
...
static int TJA1102p1_fixup_register(void)
{
	...
	if (drv) {
		dev_info(&phydev->mdio.dev, "fixup driver already loaded\n");
	} else {
		err = phy_driver_register(&nxp_TJA1102p1_fixup_driver, THIS_MODULE);
		if (err)
			goto drv_registration_error;

		pr_info("NXP PHY: Successfully registered fixup: %s\n",
...
static int __init nxp_init(void)
{
	...
	pr_info("NXP PHY: Loading driver: [%s]\n",
...
{
	pr_info("NXP PHY: unloading driver\n");
